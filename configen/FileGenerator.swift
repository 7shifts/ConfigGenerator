...
struct FileGenerator {
  ...
  fileprivate func importImplementation(for hint: OptionsParser.Hint, template: ImplementationTemplate) -> String?{
    
    guard let firstPeriodIndex =  hint.type.firstIndex(of: ".") else {
      return nil
    }
    
    let className = String(hint.type.prefix(upTo: firstPeriodIndex))
    var additionalImport =  template.implementationAdditionalImportStatements
    additionalImport.replace(token: template.additionalImportToken, withString: className)
  ...
  func generateImplementationFile(withTemplate template: ImplementationTemplate) {
    ...
    optionsParser.sortedHints.forEach { hint in
      ...
      if let header = importImplementation(for:hint, template: template),
        !additionalHeaders.contains(header)
      {
        additionalHeaders.append(header)
      }
    }
    var implemenataionAdditionalHeaders = ""
    if !additionalHeaders.isEmpty {
      implemenataionAdditionalHeaders = "\n" + additionalHeaders.joined()
    }
    ...
    do {
      let implementationOutputString = autoGenerationComment  + template.implementationImportStatements + implemenataionAdditionalHeaders + implementationBody
